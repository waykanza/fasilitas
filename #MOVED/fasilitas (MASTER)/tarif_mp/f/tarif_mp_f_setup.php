<div class="title_page">TARIF MEDIA PROMOSI<br>HALTE BUS BINTARO JAYA DAN TRANS BINTARO JAYA</div>
<div id="msg"></div>
...
function loadData()
{
	...
	jQuery('#t-detail').load(base_master_fa + 'tarif_mp/f/tarif_mp_f_load.php', data);
...
{
	var url =	base_master_fa + 'tarif_mp/f/tarif_mp_f_popup.php' +
				'?act=' + act +
				'&kode_sk=' + jQuery('#kode_sk').val() + 
				'&id=' + id,
		title	= (act == 'Simpan') ? 'Tambah' : act;
	
	setPopup(title + ' Tarif', url, 500, 300);
...
function deleteData()
{
	...
	var url		= base_master_fa + 'tarif_mp/f/tarif_mp_f_proses.php',
