<div class="title_page">DETAIL TARIF MEDIA PROMOSI<br>SPANDUK / UMBUL-UMBUL / STANDING DISPLAY</div>
<div id="msg"></div>
...
jQuery(function($) {
	...
	$('#kode_sk').on('change', function(e) {
		e.preventDefault();
		$('#t-detail').html('');
		$('#key_mp').load(base_master_fa + 'tarif_mp/c/opt_parent.php?kode_sk=' + $(this).val());
		return false;
	});
...
function loadData()
{
	...
	jQuery('#t-detail').load(base_master_fa + 'tarif_mp/c/detail_tarif_mp_c_load.php', data);
...
{
	var url =	base_master_fa + 'tarif_mp/c/detail_tarif_mp_c_popup.php' +
				'?act=' + act +
