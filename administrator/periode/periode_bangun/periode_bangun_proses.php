...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	if ($act == 'Simpan') # Proses Tambah
	{
		try
		{
			...
			ex_zero($nilai_deposit, 'Deposit harus > 0');
			...
			FROM 
				KWT_PELANGGAN p
				LEFT JOIN KWT_PERIODE_DEPOSIT d ON p.KODE_BLOK = d.KODE_BLOK AND d.TRX = '3'
			WHERE 
			...
			$id_deposit = '3#' . $periode_awal . '#' . $kode_blok;
	...
	elseif ($act == 'Ubah') # Proses Ubah
	{
		try
		{
			...
			ex_empty($periode_awal, 'Masukkan periode mulai.');
			ex_zero($jumlah_periode, 'Jumlah periode harus > 0');
			ex_zero($nilai_deposit, 'Nilai save deposit harus > 0');
