...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	try
	{
		...
		$query = "
		UPDATE PELANGGAN_MP SET STATUS = '0' WHERE NO_PELANGGAN = '$no_va'
		...
		$query = "
		INSERT INTO PELANGGAN_MP (
			NO_PELANGGAN, NAMA_PELANGGAN, NO_TELEPON, ALAMAT, NPWP,
			KODE_MP, KODE_TIPE, KODE_LOKASI, KEY_MPD, TARIF, KETERANGAN, KASIR, JUMLAH_PERIODE,
			PERIODE_AWAL, PERIODE_AKHIR,  KODE_BLOK, PEMBAYARAN,
			PERSEN_PPN, NILAI_PPN, PERSEN_NILAI_TAMBAH, NILAI_TAMBAH, PERSEN_NILAI_KURANG, NILAI_KURANG, TOTAL, TOTAL_BAYAR, 
			STATUS_BAYAR, STATUS, ID_PEMBAYARAN) 
		VALUES (
			'$no_va', '$nama_pelanggan', '$no_telepon', '$alamat', '$npwp',
			'$kode_mp', '$kode_tipe', '$kode_lokasi', '$key_mpd', $tarif, '$keterangan', '$kasir', $periode,
			CONVERT(DATETIME,'$periode_awal',105), CONVERT(DATETIME,'$periode_akhir',105), '$kode_blok', 1,
			$persen_ppn, $nilai_ppn, $persen_nilai_tambah, $nilai_tambah, $persen_nilai_kurang, $nilai_kurang, $total, $total_bayar,
			'0',1, (SELECT CASE WHEN (SELECT MAX(ID_PEMBAYARAN) FROM PELANGGAN_MP) IS NULL THEN '1' ELSE (SELECT ID_PEMBAYARAN = MAX(CASE WHEN ID_PEMBAYARAN IS NULL THEN 0 ELSE ID_PEMBAYARAN END)+1 FROM PELANGGAN_MP)end)
			)
