<?php require_once('../../../config/excel_reader.php'); ?>
...
$xls = new Spreadsheet_Excel_Reader($excel_file, false);
$baris = $xls->rowcount($sheet_index=0);

$x_tgl_bayar = clean($xls->val(1, 2));

$spl = explode('-', $x_tgl_bayar);
if ((count($spl) != 3) || ( ! checkdate($spl[1], $spl[0], $spl[2])))
{
	echo '<tr><td colspan="13">Error. Format tanggal tidak valid.</td></tr>';
	exit;
}

$tgl_bayar = $spl[0].'-'.$spl[1].'-'.$spl[2].' 00:00:00';
...
foreach ($list as $x)
{
	...
	$jumlah_bayar	= $jumlah_air + $jumlah_ipl;
	$total_bayar	= $jumlah_bayar + $biaya_admin;
