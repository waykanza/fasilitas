...
{
	$x_keterangan = (string) clean($xls->val($ix, 7));
	
	if (strpos('#' . $x_keterangan, 'UBP'))
	{
		$x_no_pelanggan = (string) clean(substr($x_keterangan, 22, 15));
...
foreach ($list as $x)
{
	...
	$tgl_bayar = $spl[0].'-'.get_int_bulan($spl[1]).'-'.$spl[2].' 00:00:00';
