...
while(!$data->EOF)
	...
	if (isset($data->fields['NO_KWITANSI'])){
		...
		jQuery(function($) {
			...
			*/
			...
			$('#hapus').on('click', function(e) {
				e.preventDefault();
				if (confirm("Apakah anda yakin data yakin hapus data  ?") == false)
				{
					return false;
				}

				var url		= base_invoice + 'invois_pkl/admin_invoice_hapus.php',
					data	= $('#form').serialize();
		...
		<input type="text" readonly="readonly" name="kode_blok" id="kode_blok" size="3" value="<?php echo $data->fields['KODE_BLOK'];?>"></tr>
