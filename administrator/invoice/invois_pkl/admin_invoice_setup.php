<div class="title-page">CETAK INVOICE PEDAGANG KAKI LIMA</div>
...
<tr>
	...
	<td>
		<select name="field" id="field">
			...
			<option value="NAMA_PELANGGAN"> SEKTOR </option>
		</select>
...
jQuery(function($) {
	...
	$(document).on('click', 'tr.onclick td:not(.notclick)', function(e) {
		...
		var periode = $(this).parent().attr('periode');
		showPopup(id, periode);
		return false;
	});	
...
function loadData()
{
	...
	jQuery('#t-detail').load(base_invoice + 'invois_pkl/invoice_load.php',data);
	
	return false;
}

function showPopup(id,periode)
{
	var url = base_invoice + 'invois_pkl/admin_invoice_popup.php?id=' + id+'&periode='+periode;
	setPopup('Invoice PKL', url, winWidth-100, winHeight-100);
