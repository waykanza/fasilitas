...
$periode_bayar	= (isset($_REQUEST['periode_num'])) ? clean($_REQUEST['periode_num']) : '';
...
$periode_awal = date("d-m-Y", strtotime($periode_awal));
...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	try
	{
		...
		*/
		...
		$query = "
		UPDATE PELANGGAN_PKL SET TARIF = $tarif, KETERANGAN = '$keterangan', UANG_PANGKAL = $uang_pangkal, PERSEN_NILAI_TAMBAH = $persen_nilai_tambah, NILAI_TAMBAH = $nilai_tambah, NILAI_KURANG = $nilai_kurang, TOTAL = $total, NILAI_PPN = $nilai_ppn, TOTAL_BAYAR = $total_bayar
		where ID_PEMBAYARAN = $id_pembayaran and MONTH(PERIODE_AWAL) = $period[1] and YEAR(PERIODE_AWAL) = $period[2];
		";

		ex_false($conn->Execute($query), $query);
		$conn->committrans();	

		$msg = "Data pelanggan kaki lima berhasil diupdate";
	}
	catch(Exception $e)
	{
		$msg = $e->getmessage();
