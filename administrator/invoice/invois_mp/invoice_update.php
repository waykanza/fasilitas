...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	try
	{
		...
		*/
		...
		$query = "
		UPDATE PELANGGAN_MP SET KETERANGAN = '$keterangan',TARIF = $tarif, PERSEN_NILAI_TAMBAH = $persen_nilai_tambah, NILAI_TAMBAH = $nilai_tambah, NILAI_KURANG = $nilai_kurang, TOTAL = $total, NILAI_PPN = $nilai_ppn, TOTAL_BAYAR = $total_bayar where ID_PEMBAYARAN = $id_pembayaran ;
		";

		ex_false($conn->Execute($query), $query);
		$conn->committrans();	
		$msg = 'Data invoice media promosi berhasil diperbarui.';
	}
	catch(Exception $e)
	{
		$msg = $e->getmessage();
