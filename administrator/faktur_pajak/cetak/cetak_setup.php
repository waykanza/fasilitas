<div class="title-page">CETAK FAKTUR PAJAK</div>
...
<tr>
	...
	<td>
		...
		<select name="tipe_cari" id="tipe_cari">
			...
			<option value="2"> SEKTOR / CLUSTER </option>
		</select>
		
		<input type="text" name="kode_blok" id="kode_blok" class="apply" value="">
		
		<select name="kode_sektor" id="kode_sektor" style="display:none;">
...
jQuery(function($) {
	...
	$(document).on('click', '#print', function(e) {
		...
		{
			open_print(base_faktur_pajak + 'cetak/cetak_proses.php?' + $('#form').serialize());
		}
...
function loadData()
{
	...
	else if (tipe_cari == '2')
	{
		if (kode_sektor == '')
		{
			alert('Masukkan sektor');
			jQuery('#kode_sektor').focus();
			return false;
		}
	}
	
	jQuery('#t-detail').load(base_faktur_pajak + 'cetak/cetak_load.php', jQuery('#form').serialize());
