<?php
require_once('pelanggan_baru_proses.php');
?>
...
jQuery(function($) {
	...
	key('alt+e', function(e) { e.preventDefault(); $('#proses').trigger('click'); });
	...
	$('#save').on('click', function(e) {
		...
		var url		= base_master + 'pelanggan_baru/pelanggan_baru_proses.php',
			data	= $('#form').serialize();
	...
	$('#proses').on('click', function(e) {
		...
		var url		= base_master + 'pelanggan_baru/pelanggan_baru_proses.php?act_proses=YES',
			data	= $('#form').serialize();
...
<table class="t-popup">
<tr>
	<td class="w10 va-top">SEKTOR</td>
	<td class="w2 va-top">:</td>
	<td class="w38 va-top">
...
<tr>
	<td class="va-top">CLUSTER</td>
	<td class="va-top">:</td>
	<td class="va-top">
		<select name="kode_cluster" id="kode_cluster">
			<option value=""> -- CLUSTER -- </option>
			<?php
			$obj = $conn->Execute("SELECT KODE_CLUSTER, NAMA_CLUSTER FROM KWT_CLUSTER ORDER BY NAMA_CLUSTER ASC");
			while( ! $obj->EOF)
			{
...
<tr>
	...
	<td class="td-action">
		...
		<input type="submit" id="proses" value=" Proses (Alt+E) ">
