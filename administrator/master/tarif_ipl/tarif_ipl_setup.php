<div class="title-page">TARIF IPL</div>

<form name="form" id="form" method="post">
<table class="t-control wauto">
<tr>
	<td width="100">NO. SK IPL</td>
	<td>
		<select name="kode_sk" id="kode_sk" class="wauto">
			<option value=""> -- NO. SK IPL -- </option>
			<?php
			$obj = $conn->Execute("SELECT KODE_SK, NO_SK FROM KWT_SK_IPL ORDER BY CAST(KODE_SK AS INT) DESC");
			while( ! $obj->EOF)
			{
...
<tr>
	...
	<td>
		...
		<select name="kode_tipe" id="kode_tipe" class="apply wauto">
			<option value=""> -- KATEGORI IPL -- </option>
			<?php
			$obj = $conn->Execute("SELECT KODE_TIPE, NAMA_TIPE FROM KWT_TIPE_IPL ORDER BY NAMA_TIPE ASC");
			while( ! $obj->EOF)
			{
...
<tr>
	...
	<td>
		<select name="field1" id="field1" class="wauto">
			<option value="KEY_IPL"> KODE </option>
			<option value="KETERANGAN"> KETERANGAN </option>
			<option value="TARIF_IPL"> TARIF IPL </option>
		</select>
...
function loadData()
{
	...
	jQuery('#t-detail').load(base_master + 'tarif_ipl/tarif_ipl_load.php', data);
...
{
	var url =	base_master + 'tarif_ipl/tarif_ipl_popup.php' +
				'?act=' + act +
				'&kode_sk=' + jQuery('#kode_sk').val() + 
				'&id=' + id,
		title	= (act == 'Simpan') ? 'Tambah' : act;
	
	setPopup(title + ' Detail Tarif IPL', url, 600, 450);
