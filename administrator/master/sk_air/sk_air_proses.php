...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	if ($act == 'Simpan') # Proses Tambah
	{
		try
		{
			...
			$query = "SELECT KODE_SK FROM KWT_SK_AIR WHERE KODE_SK = '$kode_sk'";
			ex_found($conn->Execute($query)->recordcount(), "Kode SK \"$kode_sk\" telah terdaftar.");
			
			$query = "SELECT NO_SK FROM KWT_SK_AIR WHERE NO_SK = '$no_sk'";
			ex_found($conn->Execute($query)->recordcount(), "No SK \"$no_sk\" telah terdaftar.");
			
			if ($status_sk == '1')
			{
				$conn->Execute("UPDATE KWT_SK_AIR SET STATUS_SK = '0'");
			}
			
			$query = "INSERT INTO KWT_SK_AIR (KODE_SK, NO_SK, STATUS_SK, TGL_SK, TGL_BERLAKU, PEMBUAT, KETERANGAN)
			VALUES(
				'$kode_sk', 
				'$no_sk', 
				'$status_sk', 
			...
			$msg = "SK air \"$no_sk\" berhasil disimpan.";
		}
		catch(Exception $e)
		{
			$msg = $e->getmessage();
	...
	elseif ($act == 'Ubah') # Proses Ubah
	{
		try
		{
			...
			$old_no_sk = $conn->Execute("SELECT NO_SK FROM KWT_SK_AIR WHERE KODE_SK = '$id'")->fields['NO_SK'];
			if ($no_sk != $old_no_sk)
			{
				$query = "SELECT NO_SK FROM KWT_SK_AIR WHERE NO_SK = '$no_sk'";
				ex_found($conn->Execute($query)->recordcount(), "No SK \"$no_sk\" telah terdaftar.");
	...
	elseif ($act == 'delete') # Proses Delete
	{
		...
		try
		{
			...
			{
				$query = "DELETE FROM KWT_SK_AIR WHERE KODE_SK = '$id_del'";
			...
			$msg = ($error) ? 'Sebagian data gagal dihapus.' : 'Data sk tarif air berhasil dihapus.';
		}
		catch(Exception $e)
		{
			$msg = $e->getMessage();
