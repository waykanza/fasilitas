...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	if ($act == 'Simpan') /* Proses Simpan */
	{
		try
		{
			...
			if ($status_sk == '1')
			{
				$conn->Execute("UPDATE KWT_SK_SEWA SET STATUS_SK = 0");
			}
			
			$query = "SELECT KODE_SK FROM KWT_SK_SEWA WHERE KODE_SK = '$kode_sk'";
			ex_found($conn->Execute($query)->recordcount(), "Kode SK \"$kode_sk\" telah terdaftar.");
			
			$query = "SELECT NO_SK FROM KWT_SK_SEWA WHERE NO_SK = '$no_sk'";
			ex_found($conn->Execute($query)->recordcount(), "No SK \"$no_sk\" telah terdaftar.");
			
			$query = "INSERT INTO KWT_SK_SEWA (KODE_SK, NO_SK, TGL_SK, TGL_BERLAKU, PEMBUAT, KETERANGAN, STATUS_SK)
			VALUES(
				'$kode_sk', 
				'$no_sk', 
			...
			$msg = "SK tarif sewa \"$no_sk\" berhasil disimpan.";
		}
		catch(Exception $e)
		{
			$msg = $e->getmessage();
	...
	elseif ($act == 'Ubah') /* Proses Ubah */
	{
		try
		{
			...
			$old_no_sk = $conn->Execute("SELECT NO_SK FROM KWT_SK_SEWA WHERE KODE_SK = '$id'")->fields['NO_SK'];
			if ($no_sk != $old_no_sk)
			{
				$query = "SELECT NO_SK FROM KWT_SK_SEWA WHERE NO_SK = '$no_sk'";
				ex_found($conn->Execute($query)->recordcount(), "No SK \"$no_sk\" telah terdaftar.");
			...
			$msg = 'SK tarif sewa berhasil diubah.';
		}
		catch(Exception $e)
		{
			$msg = $e->getmessage();
	...
	elseif ($act == 'delete') /* Proses Delete */
	{
		...
		try
		{
			...
			{
				$query = "DELETE FROM KWT_SK_SEWA WHERE KODE_SK = '$id_del'";
			...
			$msg = ($error) ? 'Sebagian data gagal dihapus. Kode: '.implode(', ', $id_error) : 'Data sk tarif sewa berhasil dihapus.';
		}
		catch(Exception $e)
		{
			$msg = $e->getMessage();
