...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	if ($act == 'diskon')
	{
		try
		{
			...
			$old_diskon_rupiah_air = $obj->fields['DISKON_RUPIAH_AIR'];
			...
			$diskon_rupiah_air = (isset($_REQUEST['diskon_rupiah_air'])) ? to_number($_REQUEST['diskon_rupiah_air']) : '';
			$diskon_rupiah_ipl = (isset($_REQUEST['diskon_rupiah_ipl'])) ? to_number($_REQUEST['diskon_rupiah_ipl']) : '';
			
			if ($diskon_rupiah_air != $old_diskon_rupiah_air || $diskon_rupiah_ipl != $old_diskon_rupiah_ipl)
			{
				$query_diskon = '';
				if ($diskon_rupiah_air != $old_diskon_rupiah_air)
				{
					$query_diskon .= "
					USER_DISKON_AIR = '$user_diskon',
					TGL_DISKON_AIR = GETDATE(),
					DISKON_RUPIAH_AIR = '$diskon_rupiah_air',";
				}
