...
jQuery(function($) {
	...
	$('#print').on('click', function(e) {
		e.preventDefault();
		var url = base_pembayaran + 'lain_lain/daftar_piutang_cetak.php?id=<?php echo base64_encode($no_pelanggan); ?>';
		open_print(url, '2');
	...
	$('#bayar').on('click', function(e) {
		...
		var url		= base_pembayaran + 'lain_lain/pembayaran_proses_bulk.php',
			data	= $('#form').serialize(),
			cek		= false;
		...
		$.post(url, data, function(data) {
			...
			if (data.error == false) {
				var url = base_pembayaran + 'lain_lain/cetak_kwitansi.php?idp=' + data.list_idp;
				open_print(url, '2');
				location.reload();
			}
