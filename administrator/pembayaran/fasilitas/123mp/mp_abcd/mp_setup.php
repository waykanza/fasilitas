...
function clear()
{
	...
	jQuery('#key_mpd, #tarif, #jumlah_bayar, #tgl_pemutusan').val('');
...
jQuery(function($) {
	...
	$('#kode_mp').on('change', function(e) {
		e.preventDefault();
		$('#kode_tipe').load(base_pembayaran_fasilitas + 'mp/mp_abcd/opt_kategori_mp.php?kode_mp=' + $(this).val());
		clear();
		return false;
	});
	...
	$('#kode_tipe').on('change', function(e) {
		...
		$('#kode_lokasi').load(base_pembayaran_fasilitas + 'mp/mp_abcd/opt_lokasi_mp.php?key_mp=' + key_mp);
		clear();
		return false;
	});
	
	$('#kode_tipe, #kode_lokasi, #kode_mp, #administrasi, #persen_nilai_tambah, #persen_nilai_kurang, #durasi, #luas').on('change', function(e) {
		e.preventDefault();
		calculate();
		return false;
	});
	...
	$(document).on('click', '#print', function(e) {
		...
		window.open(base_pembayaran_fasilitas + 'mp/mp_abcd/mp_print.php?id_pembayaran=' + id_pembayaran);
	...
	$(document).on('click', '#save', function(e) {
		...
		var url		= base_pembayaran_fasilitas + 'mp/mp_abcd/mp_proses.php',
			data	= $('#form').serialize();
...
<select name="kode_lokasi" id="kode_lokasi">
	<option value="0" data-key-mpd='' data-tarif=''> -- LOKASI -- </option>
</select>
</td></tr>
