<div class="title-page">PERPANJANG SEWA LOKASI PEDAGANG KAKI LIMA</div>
...
jQuery(function($) {
	...
	$(document).on('click', '#hapus', function(e) {
		...
		if (checked < 1)
		{
			alert('Pilih data PKL dahulu!');
		}
		else if (confirm('Apakah anda yakin akan memutus sewa lokasi PKL ini?'))
		{
			deleteData();
		}
		return false;
...
function loadData()
{
	...
	jQuery('#t-detail').load(base_pembayaran_fasilitas + 'pkl/perpanjang/pkl_load.php', data);
...
{
	var url =	base_pembayaran_fasilitas + 'pkl/perpanjang/pkl_popup.php' +
				'?act=' + act +
				'&id=' + id,
		title	= (act == 'Simpan') ? 'Tambah' : act;
	
	setPopup(title + ' Sewa Lokasi Pedagang Kaki Lima', url, 900, 450);
